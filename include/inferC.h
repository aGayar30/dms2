...
private:
	...
	TRTEngineSingleton() : engine(nullptr) {
		...
		engine = loadEngine("fcn-resnetGPU.engine");
		if (!engine) {
...
private:
	ICudaEngine* loadEngine(const std::string& engineFile) {
		std::ifstream file("/home/dms/DMS + AI trial/ModularCode/include/fcn-resnetGPU.engine", std::ios::binary);
		if (!file.is_open()) {
...
std::vector<std::string> classes_labels = { "Eyes Closed", "Forward", "Left Mirror", "Radio", "Rearview", "Right Mirror", "Shoulder", "Speedometer" };
