...
private:
	...
	std::string enginef="/home/dms/DMS + AI trial/ModularCode/include/runovh.engine";
        engine = loadEngine(enginef);
...
public:
    ...
    std::vector<float> infer(const std::string& imagePath) {
...
private:
    ICudaEngine* loadEngine(const std::string& engineFile) {
        ...
        std::ifstream file("/home/dms/DMS + AI trial/ModularCode/include/runovh.engine", std::ios::binary);
        if (!file.is_open()) {
